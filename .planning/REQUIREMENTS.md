...
- [x] **KB-RAG-01**: LLM 提取业绩报酬字段（`performance_fee.py` / `chapter_prompts.py`）前，自动对原文摘录进行语义检索，从 LanceDB 取 Top-K 最相似历史案例
- [x] **KB-RAG-02**: 检索结果作为 few-shot 示例注入 LLM prompt（格式：历史案例参考列表，包含字段名/字段值/原文摘录）
- [x] **KB-RAG-03**: 知识库为空时，提取流程正常运行（降级：无 few-shot 注入，不报错）
...
| KB-RAG-01 | Phase 22 | Complete |
| KB-RAG-02 | Phase 22 | Complete |
