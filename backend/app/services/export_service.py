...
def persist_export(file_id: uuid.UUID) -> tuple[str, str, str, str, str]:
    ...
    try:
        ...
        try:
            (
                ...
                subscription_path,
                export_warnings,
            ) = export_xlsx(record.extraction_result, file_id)
            ...
            record.subscription_xlsx_path = subscription_path
            ...
            session.commit()
            return product_path, fee_path, lock_path, share_path, subscription_path
...
def export_from_json(
    ...
    product, fee, lock, share, subscription, warnings = export_xlsx(extraction, fid)
    return product, fee, lock, share, subscription, warnings


def export_from_file_id(file_id: uuid.UUID) -> tuple[str, str, str, str, str, list]:
    session = SessionLocal()
    try:
        record = session.get(ContractFile, file_id)
