...
def persist_export(file_id: uuid.UUID) -> tuple[str, str]:
    ...
    try:
        ...
        try:
            product_path, fee_path, export_warnings = export_xlsx(
...
def export_from_json(json_path: str, file_id: uuid.UUID | None = None) -> tuple[str, str, list]:
    data = json.loads(Path(json_path).read_text(encoding="utf-8"))
    extraction = data.get("extraction") or data
    fid = file_id or uuid.uuid4()
    return export_xlsx(extraction, fid)


def export_from_file_id(file_id: uuid.UUID) -> tuple[str, str, list]:
    session = SessionLocal()
    try:
        record = session.get(ContractFile, file_id)
