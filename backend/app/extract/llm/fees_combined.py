...
_FEES_SYSTEM = (
    ...
    "【是否计提业绩报酬】明确否定→「否」；有业绩报酬条款→「是」；"
    "仅特定条件（侧袋/亏损等）不计提→「条件性不计提」；章节无相关内容→留空\n"
    "【业绩报酬CRM原文】提取时点/业绩报酬提取方式/业绩基准类型/门槛净值类型/提取比例"
    "若来自同一段，只在此写一次原文；各 CRM 字段填值即可\n"
    "【提取比例】业绩报酬实际提取比例：仅填纯数字（18%→「18」；0.5%→「0.5」，不含%符号）；"
    "多份额分级且各类比例不同时填「A类：18；B类：20」；合同无明确比例则留空\n"
    "【业绩基准类型】只填以下之一（不可创造新值）：\n"
    "  · 净值型：合同用高水位净值法、以基金自身净值为比较基准，不涉及外部指数\n"
    "  · 超额收益：合同明确与外部指数/业绩比较基准比较（如沪深300、中证500等）\n"
    "  无法判断则留空\n"
    "【门槛净值类型】只填以下之一（不可创造新值）：\n"
    "  · 初始水位线：门槛为 0%（初始净值1.0），即只要有正收益就计提；"
    "合同写「Ri>0%」「收益率超过0%」「正收益」均对应此项\n"
    "  · 无门槛：明确写「无门槛」「不设门槛」\n"
    "  · 分段设置：含阶梯/分段描述\n"
    "  无法判断则留空\n"
    ...
    "· 计费基准：只填以下之一：前一日资产净值 / 期间均值 / 其他；无明确说明则留空。\n"
    "  识别规则：\n"
    "  - 公式中出现「E为前一日的基金资产净值」「E为前一日份额净值」→ 前一日资产净值\n"
    "  - 分级基金逐类公式（如「EA为前一日A类份额的基金资产净值」）→ 同样填「前一日资产净值」\n"
    "  - 公式中出现「E为当期平均基金资产净值」「按区间平均」→ 期间均值\n"
    "  - 合同无计算公式、仅写「按基金资产净值X%」但未说明是哪日净值 → 留空\n"
    "  示例1：「H=E×管理费率÷365，E为前一日的基金资产净值」→ 前一日资产净值\n"
    "  示例2：「HA=EA×A类份额管理费率÷365，EA为前一日A类份额的基金资产净值」→ 前一日资产净值\n"
    "  示例3：「管理费按基金当年实际净值均值×年费率计提」→ 期间均值\n"
    "· 年计提天数：只填以下之一：365 / 360 / 实际天数；无明确则留空。\n"
    "  识别规则：\n"
    "  - 公式分母明确为「365」（如「÷365」「/365」）→ 365\n"
    "  - 公式分母明确为「360」（如「÷360」）→ 360\n"
    "  - 分母写「N」且注释为「N为当年天数」「N为当年实际天数」→ 实际天数\n"
    "  - 合同无任何公式或未提年化天数 → 留空\n"
    "  ⚠️常见陷阱：赎回费「持有365天以上免费」中的365是持有期限，不是年化天数，勿混淆。\n"
    "  示例1：「H=E×费率÷365」→ 365；示例2：「H=E×费率÷N，N为当年天数」→ 实际天数\n"
    "· 费用计算方式：只填以下之一：系统计算 / 管理人计算；无明确说明则留空。\n"
    "  识别规则：\n"
    "  - 合同含「每日计提」且给出逐日计算公式（H=E×费率÷N）→ 系统计算\n"
    "  - 合同含「自动扣划」「系统自动计提」→ 系统计算\n"
    "  - 合同含「由基金托管人负责计算」「由基金管理人自行计算」→ 管理人计算\n"
    "  - 合同只说「按季支付」「按年支付」但未提每日计提及公式 → 留空\n"
    "  示例1：合同含「H=E×费率÷365，每日计提，按季支付」→ 系统计算\n"
    "  示例2：合同写「管理费由管理人每季度末按净值自行核算后提取」→ 管理人计算"
)
    ...
    *,
    ...
    if share_class_labels:
        ...
        class_hint = (
            ...
            f"· 合同按类别展示（横向表格、逐类文字，或同一句中列举多个类别如「A类份额、C类份额」）→ {labels_str} 每个类别各输出一行（费率相同也须分行）\n"
            f"· 合同只提到部分类别有费率 → {labels_str} 中其余类别费率填0，也须各输出一行\n"
            "· 合同完全未提任何类别名称（如「本基金统一收取X%」）→ 只写一行，基金名称留空\n"
            "  ⚠️注意：只要文字中出现任何类别名称（如「A类份额、C类份额」），即视为按类别展示，必须分行输出。"
        )
    ...
    for attempt in range(settings.llm_max_retries + 1):
        try:
            ...
            perf_raw = None  # raw section now extracted by dedicated perf_fee_section call
            ...
            crm_snip = excerpt_for_display(parsed.业绩报酬CRM原文 or "") or None
